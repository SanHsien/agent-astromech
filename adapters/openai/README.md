...
| recap | ✅ | ⚠️ | 純規則,四欄回報格式原封搬。差別在**刷新那步**:Codex CLI 有 shell 與版本控制,六項刷新清單全跑得動;ChatGPT 網頁版沒有本機環境,只剩回報格式,刷新不了的項目一律標「未驗證」——這正是 skill 明文要求的行為,不是降級失敗。**ChatGPT 免動手:`prompts/recap.md` 有現成簡版。**<br>Pure rules; the four-column report ports verbatim. The difference is the *refresh* step: Codex CLI has a shell and version control, so the whole checklist runs; ChatGPT web has no local environment, leaving only the report format — anything it cannot refresh is labelled "unverified", which is exactly what the skill mandates, not a degraded failure. ChatGPT: paste the ready-made `prompts/recap.en.md`. |
| blind-review | ⚠️ | ⚠️ | 前提是「攻擊者拿不到對話」。Codex CLI 沒有子代理派工時,改用**另一個乾淨的 session**(新開一個、只餵 diff 與一句需求)——效果相同,成本是要手動搬運。ChatGPT:另開一個空白對話當攻擊者,同理。⚠️ 兩邊都要守住同一條線:**別為了方便把對話摘要貼過去**,貼了就退化成自審,整支 skill 失去意義。<br>The premise is that the attacker cannot see the conversation. Without subagent dispatch, use *another clean session* (fresh chat, fed only the diff and a one-line requirement) — same effect, manual carrying is the cost. Same for ChatGPT. ⚠️ Both paths must hold one line: **do not paste the conversation summary across for convenience** — that collapses it back into self-review. |
| token-optimizer | ⚠️ 原則通用 | ⚠️ 原則通用 | 五鐵則(分層/壓縮上報/角色鎖死/獨立驗證/失敗三停)通用;§1 模型名換成你家的檔位(如 o4-mini vs o3)。⚠️ Workflow 專屬語彙**不只 §6**——`agent()`/`schema`/`budget.remaining()`/`label` 散布在 §1 絕對規則、§2、§5、§6、§7 自檢、§8:移植時把這些讀成「你的多代理派工機制」的代稱、規則要意譯(例:「每次派工都明確指定模型」),別照抄語法。§1「進階兜底」段(settings.json/env)是 Claude Code 專屬,跳過。原作 kieiken/ultracode-token-optimization 就是 Codex 環境寫的,等於「移植回老家」。**ChatGPT 免動手:`prompts/token-optimizer.md` 有現成簡版。**<br>The five iron rules are universal; swap §1 model names for your vendor's tiers. ⚠️ Workflow-specific syntax is **not limited to §6** — `agent()`/`schema`/`budget.remaining()`/`label` appear in §1's red rule, §2, §5, §6, the §7 checklist, and §8: read them as stand-ins for *your* multi-agent dispatch mechanism and port the rules by meaning ("always pin the model when dispatching"), not by syntax. Skip §1's advanced-backstop paragraph (settings.json/env — Claude Code only). Fun fact: the upstream (kieiken) was written FOR Codex — porting it back is going home. ChatGPT: paste the ready-made `prompts/token-optimizer.en.md`. |
...
**首選:新版 Codex CLI 已有原生 skill 機制**(0.145 首次實測 2026-07,0.150.0 再驗 2026-09-06 並跑通執行層,見 `docs/TEST_PLAN.md` CROSS-05;內建 `~/.codex/skills/.system/`),直接裝:

**Preferred: recent Codex CLI has native skill support** (first verified on 0.145 in 2026-07, re-verified on 0.150.0 on 2026-09-06 including the execution layer — see `docs/TEST_PLAN.md` CROSS-05; ships `~/.codex/skills/.system/`) — just install:

```bash
npx skills add SanHsien/agent-astromech --agent codex
```
