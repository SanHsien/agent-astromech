...
> 🤖 Astromech 時刻：R2 一插進帝國終端機，讀出的是**當下**的艙門、監牢層、牽引光束狀態 ——
...
44 項行為測試，**不燒任何額度、不連任何網路**（後端全用 stub 模擬）、**不弄髒你的目錄**
...
**免費方案帳號未實測**。真實後端的 `ok` 路徑**兩條都在 Windows 驗過**：預設的 codex
內建搜尋、以及可插拔後端（後者答出比本機安裝版還新的版號＝確實查了即時網路）；
失敗分類也在真實後端上驗過（`failed_quota`）—— 沒驗過的一律別當保證。
...
換自訂後端（`AI_SEARCH_CMD`）時請注意：那條命令走 `sh -c`，路徑由**你自己**負責，
