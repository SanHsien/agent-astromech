...
| `/recap` | 平行 session 太多記不住時的現況重述：先刷新可能過期的東西，再報目標／證據／卡點／下一步 | 「recap」「現在到哪了」「重述現況」 | "recap", "where are we", "catch me up" |
...
| `/blind-review` | 派一個沒看過對話的子代理攻擊你的改動，再產出給人看的導讀 | 「對抗式審查」「攻擊這個改動」「blind review」 | "attack this change", "blind review" |
...
> 🤖 上游 `tingyulu/MyR2D2` 另附一張同內容的 4:5 圖卡（`docs/cheatsheet.png`）。本 fork **未收錄**：那張卡是 12 支的渲染圖，與本 fork 的 14 支對不上，而 fork 沒有重製它的來源檔——寧可缺一張圖，也不放一張講錯支數的圖。
